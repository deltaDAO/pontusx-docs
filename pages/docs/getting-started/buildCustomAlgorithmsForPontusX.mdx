...
3. **Set Custom Parameters**: You can provide custom parameters to configure the algorithm’s behavior within the container. These parameters are saved in a JSON file accessible to the algorithm.

### 4.1 Accessing Custom Parameters in the Algorithm

Algorithms will have access to a JSON file located at `/data/inputs/algoCustomData.json`, which contains the key-value pairs for the input data required. This allows the algorithm to use parameters specified during the publishing phase. An example of the contents of `algoCustomData.json`:
