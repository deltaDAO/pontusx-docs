...
Common choices include `python:3.15` or `node:24` from Docker Hub, but you may create a custom Docker image if your algorithm has unique dependencies.
...
<Callout type="info">
> **Best Practice**: To avoid this, specify an explicit version (e.g., `python:3.15` or `node:24`) instead of using `latest`. This approach ensures that the CtD environment remains stable and avoids unexpected failures due to container updates.
</Callout>
