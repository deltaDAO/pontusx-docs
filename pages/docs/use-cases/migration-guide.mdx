...
The GEN-X network is the soon-to-be-retired neutral decentralized physical infrastructure (DePIN) and software infrastructure for the Pontus-X ecosystem. It was developed using the Polygon Edge client and leveraging Polygon Supernets. Due to several challenges faced by the validators and participants of GEN-X it was decided to migrate to the Oasis Network for the new Pontus-X Testnet. For a comprehensive overview of the GEN-X migration including the reasons for moving to a new network and the evaluation criteria please refer to the chapter "GEN-X Migration" on page X.
...
Migrating your use cases to the Pontus-X Testnet can be achieved through two primary methods. You can either use the Pontus-X portal accessible at [pontus-x.eu](https://pontus-x.eu) or utilize [Nautilus](https://nautilus.delta-dao.com/), our comprehensive TypeScript data economy toolkit. In the following sections we will guide you through each method in detail.
